...
== bostoncamerata.org customization ==

artists has been recast as programs, with program_notes & genres added...genres introduces a dependency on a cptui, lib/genreCPTui.php, which may be included in lib/gigpress-genre.php

the repertoire page is generated by the shortcode:

    [gigpress_programs artist_order=alpha exclude=33,35,28,44,34]

the parameter 'artist_order' can be set to 'custom' to display promoted programs 1st; that order is set in the admin program edit page:

https://bostoncamerata.org/2021site/wp-admin/admin.php?page=gigpress-artists

by dragging & dropping the entries in the listing

the param 'exclude' is for hiding programs no longer performed, such as
benefits, etc...

a specific program is displayed using the program_id parameter, and specific genres by "genres=genreslug1,slug2..." (slugs are the url-safe lowercase, spaces replaced by hyphens versions of genre names)

for example, you could have a page for each genre:

    [gigpress_programs genres=christmas]

these shortcode params can be overridden in the url:

https://bostoncamerata.org/programs-repertoire/?genres=christmas,american

displays all christmas or american programs, while

https://bostoncamerata.org/programs-repertoire/?genres=christmas,american&logic=AND

displays all programs that are both christmas and american.

and of course using the search menu overrides program_id, genres & logic

==

the event page is generated by
	[gigpress_shows scope=upcoming group_artists=no condensed=0]

available shortcode params are:
			...
			'group_artists' => 'no',
			'artist_order' => 'alpha',
			'condensed' => 0,	// >0 => hide prog-note.div >1 => also hide gig-notes
			'show_menu'       => false,
			'show_menu_count' => false,
			'menu_sort'       => false,
			'menu_title'      => false,
			'year'            => false,
			'month'           => false, [1-12|current]

query params override these: 
	condensed, scope, artist
	
and add these: 
	gpy to specify year to display
	gpm to specify month to display
	artist_id 
	program_id
...
= 2.3.29.5	[26-06-05] @airdrummer
* added musicians & casts
