...
function gigpress_shows( $filter = null, $content = null ) 
{
	...
	$and_atv_conditions = '';
	if($show_id && ( !$program_id || $scope == 'past'))
	{ 
	...
	else // Not grouping by programs or showing program or only 1 program
	{
		...
		else	// No shows to display
			include gigpress_template('shows-list-empty');
	}
	
	$show_content = ( $show_menu || (! $program_id && ! $show_id) );
	echo "<div class=top-viewall>";
	if ( ($scope == 'past') && $program_id && ! $show_id)
	 	echo "<a class=viewall href='/performances/past-performances/prior-to-2014/"
		                . ( $program_id ? $program_name : '') // program_name set above or in shows-list-empty
	 	                . "'>Search for performances"
        	            . ( $program_id ? " of this program" : "")
	                    . " prior to 2014</a>";
	if( $showcontent )
        echo "<a class=viewall href=/performances/past-performances/>View Past Performances</a>";
    echo "</div>";
    
	if( $showcontent )
        echo '<div class=performances-content>' . $content . '</div>';
