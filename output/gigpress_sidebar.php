	...
	function form( $instance ) {
		...
		<p>
			...
			foreach($programs as $this_program) : ?>
				<option value="<?php echo $this_program->artist_id; ?>"
					<?php if($program_id == $this_program->artist_id) echo(' selected="selected"'); ?> >
					<?php echo gigpress_db_out($this_program->artist_name); ?>
				</option>
		  	...
		  	$tours = fetch_gigpress_tours();
				if ( $tours != false ) :
					foreach ( $tours as $this_tour ) : ?>
						<option value="<?php echo $this_tour->tour_id; ?>"
							<?php if ( $tour == $this_tour->tour_id )
								echo( ' selected="selected"' ); ?> >
							<?php echo gigpress_db_out( $this_tour->tour_name ); ?>
						</option>
			<?php endforeach; endif; ?>
			</select>
		</p>
		  	...
		  	<?php
				...
				if ( $venues != false ) :
					foreach ( $venues as $this_venue ) : ?>
						<option value="<?php echo $this_venue->venue_id; ?>"
							<?php if ( $venue == $this_venue->venue_id ) 
								echo( ' selected="selected"' );?> >
		...
		<p>
			<label>
				<input id="<?php echo $this->get_field_id('group_artists'); ?>" 
						name="<?php echo $this->get_field_name('group_artists'); ?>" 
						type="checkbox" value="yes"<?php if($group_artists == 'yes') echo ' checked="checked"'; ?> /> 
		...
		<p>
			<select style="width:100%;" id="<?php echo $this->get_field_id('artist_order'); ?>" 
				...
				<option value="alphabetical"
					<?php if($artist_order == 'alphabetical') echo ' selected="selected"'; ?> >
					<?php _e("Order programs alphabetically", "gigpress"); ?>
				</option>
				<option value="custom"
					<?php if($artist_order == 'custom') echo ' selected="selected"'; ?> >
...
function gigpress_sidebar( $filter = null ) {
	...
	if($group_artists && !$tour && !$program_id && !$venue && $total_programs > 1) { 
		...
		foreach($programs as $program_group) {
			...
			$shows = $wpdb->get_results("SELECT * FROM " . GIGPRESS_ARTISTS . " AS a, "
														 . GIGPRESS_VENUES . " as v, " 
														 . GIGPRESS_SHOWS ." AS s 
											LEFT JOIN  " . GIGPRESS_TOURS . " AS t 
													ON s.show_tour_id = t.tour_id 
											WHERE " . $date_condition . " AND show_status != 'deleted' AND s.show_artist_id = " 
													. $program_group->artist_id . " AND s.show_artist_id = a.artist_id AND s.show_venue_id = v.venue_id " 
													. $further_where . " ORDER BY s.show_date " . $sort . ",s.show_expire " . $sort . ",s.show_time " . $sort 
													. " LIMIT " . $limit);
		...
		$shows = $wpdb->get_results( "SELECT * FROM " . GIGPRESS_ARTISTS . " AS a, " 
													  . GIGPRESS_VENUES . " as v, " 
													  . GIGPRESS_SHOWS . " AS s LEFT JOIN  " 
										. GIGPRESS_TOURS . " AS t ON s.show_tour_id = t.tour_id WHERE " . $date_condition 
